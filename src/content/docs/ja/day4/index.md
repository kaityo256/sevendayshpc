---
title: "Day 4: 領域分割による非自明並列"
description: 一次元拡散方程式を領域分割して並列化する
sidebar:
  order: 6
---
...
![馬鹿パラの特徴](/sevendayshpc/ja/day4/fig/bakapara.png)
...
![非自明並列の通信パターン](/sevendayshpc/ja/day4/fig/nontrivial.png)

毎回全ノードと計算が必要になるタイプは、典型的には高速フーリエ変換が必要となる場合である。
例えば、「円周率を一兆桁計算する」といった計算において、多倍長計算が必要となり、その多倍長計算の実行にフーリエ変換が使われる。乱流の計算にもフーリエ変換が使われる。以前、地球シミュレータで大規模な乱流シミュレーションが行われたが、これは地球シミュレータの強力なネットワークがなければ実現が難しかった。こういう全ノード通信は、バタフライ型のアルゴリズムで実行されることが多いが、ここでは深入りしない。興味のある人は「並列FFT」などでググってみて欲しい。
...
![加熱シミュレーションの結果](/sevendayshpc/ja/day4/fig/thermal.png)
...
![一様加熱シミュレーションの時間発展](/sevendayshpc/ja/day4/fig/uniform.png)
...
![固定境界シミュレーションの時間発展](/sevendayshpc/ja/day4/fig/fixed.png)
...
![並列プログラムにおけるファイルの吐き方](/sevendayshpc/ja/day4/fig/parafile.png)
...
![通信の「のりしろ」](/sevendayshpc/ja/day4/fig/margin.png)
...
![RendezvouzプロトコルとEagerプロトコル](/sevendayshpc/ja/day4/fig/r_and_e.png)

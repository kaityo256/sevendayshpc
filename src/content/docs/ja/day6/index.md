---
title: "Day 6: ハイブリッド並列"
description: MPIとOpenMPを組み合わせたハイブリッド並列を学ぶ
sidebar:
  order: 8
---
...
TLBもキャッシュの一種であるから、通常のキャッシュと同様にキャッシュミスを起こす。TLBミスが起きると、CPUはページテーブルをたどって、仮想アドレスに対応する物理アドレスを調べる必要がある。

64 bitのアドレスをすべて使えば、理論上は$2^{64}$バイト、すなわち16 EiBもの仮想アドレス空間を表現できる。しかし、実際のx86-64ではすべての64ビットをアドレス変換に使うわけではない。ここでは典型的な4段ページングを考え、下位48ビットを使う場合を考えよう。

この48ビットは5つの部分に分けられる。最下位の12ビットは、ページ内のどの場所にあるかを表すオフセットである。12ビットで$2^{12}=4096$バイトを表せるので、通常のページサイズは4 KiBとなる。残りの36ビットは9ビットずつ4つに分けられ、それぞれページテーブルの各階層をたどるために使われる。住所にたとえるなら、4つの9ビットはそれぞれ「東京都」「千代田区」「一丁目」「一番地」のような階層に対応する。最後の12ビットは、その建物の中の部屋番号に相当する。

仮想アドレスが与えられると、CPUはまず最上位の9ビットを使って最初の住所録を引く。そこには、次の住所録が物理メモリのどこに置かれているかが書かれている。次の9ビットを使ってその住所録を引き、さらに次の住所録へ進む。この操作を4段階繰り返すと、最終的に目的の物理ページがどこにあるかが分かる。このように、ページテーブルを順番にたどって仮想アドレスを物理アドレスへ変換する処理を **ページウォーク** と呼ぶ。

一方、2 MiBのラージページを使う場合には、通常なら最後のページテーブルを選ぶために使う9ビットもページ内オフセットとして使う。するとオフセットは12+9=21ビットとなり、1ページのサイズは$2^{21}=2097152$バイト、すなわち2 MiBとなる。住所の例でいえば、住所の階層を一段階減らし、その代わり最後に指定される建物を巨大な超高層ビルにしたようなものである。ラージページを使うと、1つのTLBエントリでより広いメモリ領域をカバーできるため、同じ大きさのTLBでもTLBミスを減らすことができる。
...
![メモリとCPUの接続関係](/sevendayshpc/ja/day6/fig/numa.png)
...
![ファーストタッチの原則](/sevendayshpc/ja/day6/fig/firsttouch.png)
...
![perfの実行結果](/sevendayshpc/ja/day6/fig/perf_sample.png)
...
![ディレクティブを内側に入れた場合](/sevendayshpc/ja/day6/fig/inner.png)

青が実際に計算しているところで、赤がオーバーヘッドである。計算時間は順調に減っているのだが、実行時間のほとんどをオーバーヘッドが占めるという、かなり悲惨なことになっている。

次に、外側にディレクティブを入れた場合。

![ディレクティブを外側に入れた場合](/sevendayshpc/ja/day6/fig/outer.png)
...
![スレッド並列実行時のperfの実行結果](/sevendayshpc/ja/day6/fig/perf.png)
...
![バリア命令のアセンブリ](/sevendayshpc/ja/day6/fig/perf2.png)
...
![シングルノードでのスケーリング結果](/sevendayshpc/ja/day6/fig/single_scaling.png)
...
![マルチノードでのスケーリング結果](/sevendayshpc/ja/day6/fig/multi.png)

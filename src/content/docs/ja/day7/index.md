---
title: "Day 7: SIMD化"
description: SIMD命令とデータ構造による高速化を学ぶ
sidebar:
  order: 9
---
...
![CPUにおける並列実行形式](/sevendayshpc/ja/day7/fig/simd.png)
...
SIMDレジスタの内容をメモリに保存する命令が`_mm256_store_pd`である。
...
![磁場中の荷電粒子の軌道](/sevendayshpc/ja/day7/fig/one.png)
...
![オイラー法とルンゲクッタ法によるエネルギーの保存の様子](/sevendayshpc/ja/day7/fig/energy.png)
...
![SIMD命令によるデータのロード](/sevendayshpc/ja/day7/fig/load_pd.png)
...
![permuteによる要素の入れ替え](/sevendayshpc/ja/day7/fig/permute.png)

これによりレジスタの中身を適宜並び替えて、足したりかけたり引いたりすればよろしい。磁場については、使うベクトルが(z,x,y,0)と(y,z,x,0)のパターンしかないので、最初に宣言しておこう。
